...
def convertCandlesForFun(candles):
    pass

def getCandleInfo(candles, doji_size=0.05):
    info = []
    doji_size = max(0.01, doji_size)



    if abs(open - close) <= (high - low) * doji_size:
        info.append({ 'name': 'Doji', 'color': 'white', 'trend': 'default' })
    
    if close[2] > open[2] and min(open[1], close[1]) > close[2] and open < min(open[1], close[1]) and close < open:
